    ...
    <div>
      <div className='mr-2' >
        <nav className="text-gray-350  ">
          <div className=" flex flex-wrap items-center justify-between px-4 py-9 sm:p-8 rounded-xl border bg-white">
            <div className="flex items-center drop-shadow-lg">
              {
                <Link to={link}>
                  <div className='flex items-center gap-1'>
                    ...
                    <span className="self-center text-xl sm:text-2xl font-semibold whitespace-nowrap capitalize bg-white ">{header}</span>
                  </div>
                </Link>
              }
            </div>
            <div className="relative bg-white" id="navbar-default">
              <div className='flex flex-row'>
                <div onClick={e => setShowMenue(true)} className='rounded-full drop-shadow-lg bg-white flex justify-center items-center  mr-1 sm:mr-4 w-9 h-9'>
                  <PiBellLight className='w-6 h-6' />
                </div>
                <button type="button" className="flex text-sm rounded-full md:mr-0 bg-white" onClick={e => setDrop(!drop)} >
                  <div className='flex items-center text-sm drop-shadow-lg bg-white'>
              ...
              <div className={`z-50 ${drop ? null : 'hidden'} absolute w-full px-4 my-4 text-gray-950 font-medium list-none bg-white backdrop-blur-md bg-opacity-10 divide-y divide-gray-100 rounded-lg shadow`}>
                <ul className="py-2" aria-labelledby="user-menu-button">
                  <li>
                    <Link to="/profile" className="block px-4 py-2 text-sm  text-white rounded-md  "  style={{ backgroundColor: '#836852' }}>Profile</Link>
                  </li>
                </ul>
              </div>
              {/* </div> */}
